...
export class UserArgument extends Argument<Command> {
	public async run(parameter: string, context: CommandArgumentContext) {
		...
		if (found) {
			return this.isAllowed(found, context) ? this.ok(found) : this.error({ parameter, identifier: `I could not resolve ${parameter} to a command! Make sure you typed its name or one of its aliases correctly!`, context });
		}
		...
		return this.error({ parameter, identifier: `I could not resolve ${parameter} to a command! Make sure you typed its name or one of its aliases correctly!`, context });
	}
