...
function Show-Help {
	Write-Host ""
	Write-Host "Pyne Docker Control Script" -ForegroundColor blue
	Write-Host ""
...
function Step-Run {
	...
	Process {
		switch ($Command) {
			...
			stop { docker-compose -p pyne -f "$($PSScriptRoot)/docker-compose.yml" down $Service }
			logs { docker-compose -p pyne -f "$($PSScriptRoot)/docker-compose.yml" logs $Service }
			tail { docker-compose -p pyne -f "$($PSScriptRoot)/docker-compose.yml" logs -f $Service }
			push { docker push $Service }
			remove { docker-compose -p pyne -f "$($PSScriptRoot)/docker-compose.yml" rm -fv $Service }
			update { docker-compose -p pyne -f "$($PSScriptRoot)/docker-compose.yml" pull $Service; docker-compose -p pyne -f "$($PSScriptRoot)/docker-compose.yml" up -d --force-recreate $Service }
